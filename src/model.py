...
class GPTConfig:
    ...
    def __init__(self, vocab_size, block_size, vocab_midi_size, **kwargs):
        self.vocab_size = vocab_size
        self.block_size = block_size
        self.vocab_midi_size = vocab_midi_size
        self.use_eigenspace = kwargs.pop('use_eigenspace', False)
        self.n_eigen = kwargs.pop('n_eigen', 4)  # (α, β, γ, δ)
...
class MidiEmbedding(nn.Module):
    def __init__(self, n_embd, midi_vocab=128, dm=4):
        super().__init__()
        self.embed = nn.Embedding(midi_vocab, embedding_dim=dm) #midi_vocab = 128 
        self.lin_m = nn.Linear(8*dm, n_embd, bias=False) #dm * 8 (is the midi notes)
        
    def forward(self, m):
        midi = self.embed(m)
        midi = midi.view(m.size(0), midi.size(1), -1)
        return self.lin_m(midi)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.number_embeddings = config.n_embd
        self.vocab_midi_size = config.vocab_midi_size
        #Emebeddings section   
    ...
    def forward(self, idx, targets=None, midi=None, eigen=None):
        ...
        acc_0= torch.softmax(logits, dim=1)
        print('acc_0:', acc_0)
        acc = (targets == y_pred).sum() / targets.size(0)
        acc = acc.item()
        print('acc_1:', acc)
        '''
