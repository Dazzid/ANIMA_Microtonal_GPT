...
pack_data_b.py
==============
Phase-B data packer: paired ``(.txt, .mid, .eigen.npy)`` → train/val ``.bin``
using the Method-B tokenizer (``tokenizer_b.TokenizerB``).

Mirror of ``pack_data_v3.py`` with two changes:

  1. Tokenizer: ``MPETokenizer`` → ``TokenizerB`` (holdrian ``H_<step>_<vel>``
     instead of MIDI-derived ``PV_<step>_<vel>``).
  2. Output directory: ``dataset/tokenized/`` → ``dataset/tokenized_b/``.

Everything else (L1 interleave, EigenSpace span expansion under Visibility
Rule B, chunking, train/val split, streaming eigen stats, meta.json shape)
is identical byte-for-byte. See ``STRATEGY_V3.md`` §B.2 / §B.3 and the
isolation contract §B.0 (rules 1, 3, 4, 6):

  * new file with ``_b`` suffix, no edits to any Method-A module,
  * writes only to ``dataset/tokenized_b/``,
  * consumes ``.eigen.npy`` sidecars read-only,
  * ``TokenizerB`` is a standalone class — no subclass of ``MPETokenizer``.

Usage
-----
    python pack_data_b.py                         # full dataset
    python pack_data_b.py --max-files 100         # quick sanity
    python pack_data_b.py --block-size 2048       # shorter context
    python pack_data_b.py --workers 16
    python pack_data_b.py --regen-meta-only       # rebuild meta from bins
"""
...
# =============================================================================
# CONSTANTS — must agree with model.py
# =============================================================================
...
def _worker_init():
    import tokenizer_b as _tokmod
    _process_pair._tok = _tokmod.TokenizerB()
...
def _eigen_stats_streaming(path: Path, seq_len: int, n_dims: int = 4,
    ...
    for s in range(0, n_seqs, chunk_seqs):
        blk = mm[s:s + chunk_seqs].astype(np.float32)
        diff = blk.reshape(-1, n_dims) - means
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default=str(_ROOT_DIR / "dataset" / "tokenized_b"))
    ...
    # Isolation-contract safety: refuse to clobber Method A's output tree.
    forbidden = _ROOT_DIR / "dataset" / "tokenized"
    if output_dir.resolve() == forbidden.resolve():
        raise SystemExit(
            f"refusing to write into Method-A directory {forbidden}. "
            f"Use a different --output-dir (default is dataset/tokenized_b)."
        )

    print("=" * 72)
    print("PACK B — paired (.txt, .mid, .eigen.npy) → .bin  [TokenizerB]")
    print("=" * 72)
    ...
    if args.regen_meta_only:
        ...
        _write_meta(
            ...
            n_songs_total=n_train_seqs + n_val_seqs,
    ...
    train_tokens = np.stack(tr_tok)
    train_eigen = np.stack(tr_eig)
